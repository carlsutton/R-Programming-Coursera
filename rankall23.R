...
HA1 <- data.table(hosp,st,HALME,rankha1)
ha1 <- HA1[complete.cases(HA1),]
ha1
unique.halme <- unique(ha1$HALME)
rankha1 <- apply(ha1,1,rank(ha1$rankha1, ties.method = "first"))
rankha1
rankha <- rank(unique.halme, ties.method = "first")
rankha
